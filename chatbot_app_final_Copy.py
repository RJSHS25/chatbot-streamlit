...
class SimpleChatbot:
    def __init__(self, questions_file, label_mapping_file, model_file, type_encoder_file, customer_encoder_file):
        self.load_files(questions_file, label_mapping_file, model_file, type_encoder_file, customer_encoder_file)
    ...
    def load_files(self, questions_file, label_mapping_file, model_file, type_encoder_file, customer_encoder_file):
        files_to_check = [questions_file, label_mapping_file, model_file, type_encoder_file, customer_encoder_file]
        ...
        self.label_mapping = pd.read_csv(label_mapping_file)
        self.model = joblib.load(model_file)
        self.label_encoder_type = joblib.load(type_encoder_file)
        self.label_encoder_customer = joblib.load(customer_encoder_file)
    ...
    def suggest_questions(self, input_text, cosine_threshold=0.3, fuzzy_threshold=70):
        ...
        fuzzy_suggestions = [q for q in self.unique_questions if fuzz.ratio(processed_input, q) >= fuzzy_threshold]
    ...
    def predict_next_questions(self, last_question):
        ...
        if not matched_row.empty:
            next_questions = [
                matched_row['Next Question 1'].values[0] if not pd.isnull(matched_row['Next Question 1'].values[0]) else '',
                matched_row['Next Question 2'].values[0] if not pd.isnull(matched_row['Next Question 2'].values[0]) else '',
                matched_row['Next Question 3'].values[0] if not pd.isnull(matched_row['Next Question 3'].values[0]) else ''
            ]
